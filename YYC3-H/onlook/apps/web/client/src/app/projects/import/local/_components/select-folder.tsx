...
export const NewSelectFolder = () => {
        ...
        if (packageJsonFile) {
            try {
                if (typeof packageJsonFile.content === 'string') {
                    const packageJson = JSON.parse(packageJsonFile.content) as { name?: string };
    ...
    const filterAndProcessFiles = async (files: File[]): Promise<ProcessedFile[]> => {
        ...
        const allPaths = files.map((file) => {
            // 类型保护，避免 any
            return Object.prototype.hasOwnProperty.call(file, 'webkitRelativePath')
        ...
        for (const file of files) {
            ...
            if (Object.prototype.hasOwnProperty.call(file, 'webkitRelativePath')) {
                    ...
                    for (const entry of entries) {
                        if ((entry as FileSystemFileEntry).isFile) {
                            ...
                            try {
                                ...
                                console.warn(`Error reading file ${(entry as FileSystemFileEntry).name}:`, error);
                            }
            ...
            if (item && item.kind === 'file') {
                ...
                if (entry) {
                    if (entry.isFile) {
                        ...
                        // 类型断言为 FileSystemDirectoryEntry
