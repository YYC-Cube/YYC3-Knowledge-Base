/**
 * YYC3 Offline Store — Unit Tests
 * @description 验证网络状态检测、缓存管理 (LRU/TTL/按类别)、同步队列、策略配置、面板开关
 * @version 4.8.0
 * 对齐 Guidelines: Offline-First — cache strategies, sync queue, connectivity detection
 */
...
describe('Cache Management', () => {
  ...
  it('LRU eviction should remove oldest entry when cache exceeds limit', () => {
    ...
    const calls = lsMock.setItem.mock.calls.filter((c: string[]) => c[0] === 'yyc3_offline_store')
    const parsed = JSON.parse(calls[calls.length - 1][1])
    const keys = parsed.cacheEntries.map((e: CacheEntry) => e.key)
...
describe('Sync Queue', () => {
  ...
  it('addToSyncQueue should add a pending item', () => {
    ...
    const calls = lsMock.setItem.mock.calls.filter((c: string[]) => c[0] === 'yyc3_offline_store')
    const parsed = JSON.parse(calls[calls.length - 1][1])
    const pending = parsed.syncQueue.filter((q: SyncQueueItem) => q.status === 'pending')
