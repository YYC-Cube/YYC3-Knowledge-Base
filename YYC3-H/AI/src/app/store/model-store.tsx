...
export function ModelStoreProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const doCheck = () => {
      ...
      fetch(pingUrl, { method, headers: hdrs, body, signal: ctrl.signal })
        ...
        .catch((err: any) => {
          clearTimeout(tm);
          setConnectivityMap((p) => ({ ...p, [mid]: { modelId: mid, status: "offline", lastChecked: Date.now(), error: err?.name === "AbortError" ? "Timeout" : (err?.message || "Failed") } }));
        });
    };
  ...
  const checkConnectivity = useCallback(async (modelId: string): Promise<ConnectivityStatus> => {
    ...
    try {
      ...
      const r: ConnectivityStatus = { modelId, status: "offline", lastChecked: Date.now(), error: err?.name === "AbortError" ? "Timeout" : (err?.message || "Failed") };
      setConnectivityMap((p) => ({ ...p, [modelId]: r }));
      return r;
    }
  }, []);
  ...
  const testModel = useCallback(async (modelId: string): Promise<ModelTestResult> => {
    ...
    try {
      ...
      setConnectivityMap((p) => ({ ...p, [modelId]: { modelId, status: "offline", lastChecked: Date.now(), error: err?.message } }));
      return { success: false, latencyMs: 0, error: err?.message || "Unknown error", model: model.name };
    }
  }, []);
  ...
  const sendToActiveModel = useCallback(async (message: string, options?: { systemPrompt?: string; history?: { role: string; content: string }[] }): Promise<string> => {
    ...
    const tryModel = async (m: AIModel): Promise<string> => {
      ...
      try {
        ...
        setConnectivityMap((p) => ({ ...p, [m.id]: { modelId: m.id, status: "offline", lastChecked: Date.now(), error: err?.message } }));
        throw err;
      }
    };
    ...
    try {
      ...
      for (const fb of fallbacks) {
        try {
          console.log(`[YYC³ Fallback] Trying fallback model: ${fb.name}`);
