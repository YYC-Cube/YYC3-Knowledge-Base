/**
 * @file IDEMode.tsx
 * @description YYC3 IDE main shell — orchestrates layout, context, and sub-components
...
export function IDEMode({ onSwitchMode, onOpenSettings, onOpenNotifications, onOpenCommandPalette, onOpenGlobalSearch }: {
  ...
  }, [projectStore.activeProjectId, projectStore.projects]);
  ...
  // Sync state → Zustand
  useEffect(() => { ideStore.setLayoutMode(viewMode); }, [viewMode]);
  useEffect(() => { ideStore.setFullscreenPreview(fullscreenPreview); }, [fullscreenPreview]);
  useEffect(() => { ideStore.setLeftWidth(resize.leftWidth); }, [resize.leftWidth]);
  useEffect(() => { ideStore.setMiddleRatio(resize.middleRatio); }, [resize.middleRatio]);
  useEffect(() => { ideStore.setTerminalVisible(terminalVisible); }, [terminalVisible]);
  useEffect(() => { ideStore.setTerminalHeight(resize.terminalHeight); }, [resize.terminalHeight]);
