...
创建腾讯云工单。必填参数：`Content`（问题描述）、`CategoryId`（三级分类 ID，从 GetMCCategoryListById 获取）。

- **触发词**："创建工单"、"提交工单"、"新建工单"、"我要提工单"、"create ticket"
- **工作流**：
  1. 调用 `GetMCCategoryList` 获取一级/二级分类列表
  2. 根据用户问题描述，匹配最佳二级分类
  3. 调用 `GetMCCategoryListById(Level2Id=二级分类ID)` 获取三级分类列表
  4. 从三级分类中匹配最佳问题类型
...
### 3.11 GetMCCategoryListById — 获取三级分类列表
