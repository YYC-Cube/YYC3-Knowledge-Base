...
本服务使用 JWT Bearer Token 鉴权。**Agent 必须在调用查询脚本之前，通过 `connect_cloud_service` 工具获取 token。**

### 步骤

1. **调用 `connect_cloud_service` 工具**（无需参数）获取当前会话的 JWT token
2. 从返回结果的 `token` 字段中提取完整的 JWT 字符串
3. 将该 token 通过 `--token` 参数传递给查询脚本

> ⚠️ 不要向用户展示或提及 token 的具体内容，静默获取后直接使用即可。
...
代理会自动填充 `channel`、`sub_channel`、`request_id` 等固定字段，客户端只需提供查询内容。
...
**完整调用流程**：
```
1. 调用 connect_cloud_service 工具 → 得到 token
2. 执行: python scripts/query.py --query "用户的查询" --token "<获取到的token>"
3. 解析返回的 JSON，向用户呈现金融数据
```

**Python（推荐）**：
```bash
python scripts/query.py --query "腾讯最新财报" --token "<从connect_cloud_service获取的token>"
python scripts/query.py --query "贵州茅台股价" --data-type api --token "$TOKEN"
python scripts/query.py --query "黄金价格" --token "$TOKEN"
```

**Shell（备选）**：
```bash
bash scripts/query.sh --token "<token>" "腾讯最新财报"
bash scripts/query.sh --token "$TOKEN" "贵州茅台股价"
```

## 请求参数

客户端请求体只需提供以下字段（代理自动填充其余固定字段）：

| 字段 | 必填 | 说明 |
|------|------|------|
| `query` | 是 | 自然语言查询，如"腾讯最新财报" |
| `data_type` | 否 | `all`=API+文章（默认）；`api`=仅结构化数据；`doc`=仅文章 |
| `sub_channel` | 否 | 子渠道信息，可选覆盖 |
