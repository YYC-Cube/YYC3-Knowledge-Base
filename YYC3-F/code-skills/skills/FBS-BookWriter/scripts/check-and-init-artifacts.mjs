...
export async function checkAndInitArtifacts(bookRoot, skillRoot) {
  ...
  if (missingFiles.length === 0) {
    console.log('✅ [U1-P0] 所有工件已就绪');
    return true;
  }
  
  // 4. 有缺失,执行初始化
  console.log(`⚠️ [U1-P0] 缺失 ${missingFiles.length} 个工件,正在初始化...`);
  ...
  try {
    ...
    console.log('✅ [U1-P0] 工件初始化执行完成');
    
    // 5. 验证初始化结果
    const stillMissing = REQUIRED_ARTIFACTS.filter(file => 
      !fs.existsSync(path.join(bookRoot, file))
    );
    
    if (stillMissing.length > 0) {
      throw new Error(
        `[U1-P0] 工件初始化失败,仍缺失: ${stillMissing.join(', ')}`
      );
    }
    
    console.log('✅ [U1-P0] 所有工件初始化成功');
    ...
    console.error('❌ [U1-P0] 工件初始化失败:', error.message);
    console.error(error);
    throw error;
  }
}
