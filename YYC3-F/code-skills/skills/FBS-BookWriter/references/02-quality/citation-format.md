...
> **定位**：解决多 Writer 并行时「各选各的引用格式」导致的混用问题。与 `quality-check.md` **G4**、`search-policy.json` **chapterWriting**、`section-3-workflow.md` **S3** 对齐。  
> **版本**：1.7.0

**HTML 脚注**：终稿由 `assets/build.mjs` 输出时，技能根 **`npm install`** 已锁定 **`markdown-it-footnote`**；未安装则构建 **直接失败**（见 `html-deliverable-gate.md` §4）。

**S3 执行约束**（测试报告 04）：主编须在 **每个 Writer 任务** 中粘贴 [`workbuddy-agent-briefings.md`](../01-core/workbuddy-agent-briefings.md) 的 **「数据引用格式广播块」**；合稿前可跑 `scripts/citation-format-check.mjs`。
...
| **检索可审计** | 每章仍须满足 `minQueriesPerChapter`；检索痕迹见 `scripts/enforce-search-policy.mjs` 认可的区块（含 §3）。 |
