...
# Validate cuFOLIO agent skills and plugin manifests.
...
for dir in "$SKILLS_DIR"/*/; do
  ...
  if ! grep -q '^license:' "$skill_md"; then
    echo "ERROR: $name/SKILL.md missing license in frontmatter"
    ERRORS=$((ERRORS + 1))
  fi
  if [ "$(basename "$skill_md" .md)" != "SKILL" ]; then
...
if [ -f "$AGENTS_MD" ]; then
  ...
  for dir in "$SKILLS_DIR"/*/; do
    ...
    if ! grep -q "$name" "$AGENTS_MD"; then
      echo "ERROR: AGENTS.md does not reference skill: $name"
      ERRORS=$((ERRORS + 1))
    fi
  done
fi
